...
export const Dashboard = ({ mode = 'default' }: { mode?: AppMode }) => {
  ...
  const getTitle = (base: string) => {
    if (mode === 'water') return '水务' + base
    if (mode === 'electric') return '电力' + base
    return base
  }
  ...
  return (
    <>
      ...
      <div className="absolute top-[15%] left-4 w-80 flex flex-col gap-4">
        <Card title={mode === 'default' ? "社区能耗趋势" : (mode === 'water' ? "水流趋势" : "电力趋势")} className="h-64">
      ...
      <div className="absolute top-[15%] right-4 w-80 flex flex-col gap-4">
        ...
        <Card title={mode === 'default' ? "人车流量监控" : "区域用量分布"} className="h-56">
          <ReactECharts option={thirdOption} style={{ height: '100%', width: '100%' }} />
        </Card>
      </div>
    </>
  )
